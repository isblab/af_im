...
# 		multistate_dict = self.run_multistate_pipeline()
...
# 				multistate_dict = multistate_dict
# 			)
# 			multistate_dict = self.get_mmcif_metadata_for_multistate(
# 				multistate_dict = multistate_dict
# 			)
# 			multistate_dict = self.extract_chains_from_struct(
# 				multistate_dict = multistate_dict
# 			)
...
# 			xls_df1 = map_xls_to_seq_id(
# 				xl_df = xls_df1,
# 				pdb_num_seq_id_map = multistate_dict[sys_name]["apo"]["mapping"]
# 			)
# 			xls_df2 = map_xls_to_seq_id(
# 				xl_df = xls_df2,
# 				pdb_num_seq_id_map = multistate_dict[sys_name]["holo"]["mapping"]
# 			)
# 			xls_df1 = self.map_chains_to_prot(
# 				sys_name = sys_name,
# 				xls_df = xls_df1
...
# 			xls_s1_2 = xls_s1_2.reset_index( drop = True )
# 			# No XLs left
# 			if xls_df1.shape[0] == 0 or xls_df2.shape[0] == 0:
# 				remove.append( sys_name )
# 			else:
...
# 	) -> pd.DataFrame:
# 		"""
# 		We select entries form the benchmark which:
# 			Involve domain movements.
# 			Have a rmsd_apo_holo >10 angstrom.
# 		Return the filtered dataframe.
# 		"""
...
# 			# For NMR structures, the model ID is mentioned; excludin
# 			if "-" in apo_pdb or "-" in holo_pdb:
# 				continue
...
# 	################################################################################
# 	def get_mmcif_metadata_for_multistate(
# 		self,
# 		multistate_dict: Dict[str, str]
# 	) -> Dict[str, Any]:
# 		"""
# 		For the selected structures in the multistate benchmark,
# 			obtain the following metadata from the MMCIF file,
# 			SEQRES Sequence
# 			PDB residue numbering
# 			seq_id numbering
# 		We further need the experimental resolution of the structure.
# 		Obtain mapping between the PDB numbering to seq_id.
# 		"""
# 		print( "\nExtracting metadata from MMCIF file..." )
# 		seqres_dict = self.get_metadata_from_mmcif(
# 			multistate_dict = multistate_dict
# 		)
# 		resolution_dict = self.get_pdb_resolution(
# 			multistate_dict = multistate_dict
# 		)

# 		pdb_seq_id_mapping = create_pdb_num_to_seq_id_mapping(
# 			seqres_dict = seqres_dict
# 		)

# 		# Add the required metadata from seqres_dict to multistate_dict.
# 		for sys_name in multistate_dict:
# 			for state in ["apo", "holo"]:
# 				pdb_id = multistate_dict[sys_name][state]["pdb_id"]
# 				# These are _asym_id's
# 				chain_id = multistate_dict[sys_name][state]["chain_id"]

# 				mmcif_obj = MmcifDictParser(
# 					cif_file = os.path.join( self.dir_paths["pdb_dir"], f"{pdb_id}.cif" )
# 				)
# 				asym_auth_asym_map = mmcif_obj.get_chain_mapping()

# 				# We need the auth_asym_id
# 				auth_asym_chain_id = asym_auth_asym_map["asym"][chain_id]

# 				for entity_id in seqres_dict[pdb_id]:
# 					if auth_asym_chain_id not in seqres_dict[pdb_id][entity_id]:
# 						continue
# 					chain = seqres_dict[pdb_id][entity_id][auth_asym_chain_id]
# 					for k in chain:
# 						multistate_dict[sys_name][state][k] = chain[k]
# 					# Also add the PDB num to seq_id mapping.
# 					chain_mapping = pdb_seq_id_mapping[pdb_id][auth_asym_chain_id]
# 					multistate_dict[sys_name][state]["mapping"] = {auth_asym_chain_id: chain_mapping}
# 		return multistate_dict

# 	################################################################################
# 	def get_metadata_from_mmcif(
# 		self,
# 		multistate_dict: Dict[str, str]
# 	):
# 		"""
# 		Extract relevant metadata from the MMCIF file.
# 		Some entries may correspond to a larger complex.
# 			But we are only interested in a single chain,
# 				and just need the metadata like resolution
# 				and sequence.
# 			We just set arbitrary total_length and frac_coverage
# 				to prevent the entries from being excluded.
# 		"""
# 		pdb_ids_list = []
# 		for sys_name in multistate_dict:
# 			for state in ["apo", "holo"]:
...
# 		seqres_obj = SeqResDict(
# 			pdb_ids_list = pdb_ids_list,
# 			pdb_struct_dir = self.dir_paths["pdb_dir"],
# 			cores = self.cpu_cores,
# 			max_sys_length = 10000,
# 			frac_coverage = 0.95,
# 		)
# 		seqres_obj.exclude_monomer = False
# 		seqres_obj.forward()

# 		seqres_dict = seqres_obj.seqres_dict
...
# 		When downloading the biological assembly, the cif file
# 			does not contain the resolution of the structure.
# 		So we need to get the resolutions for all separately
# 			from the PDB REST API.
# 		"""
# 		print( "\nFetching the resolution for biological assemblies..." )
# 		resolution_dict = {}
...
# 	################################################################################
# 	def extract_chains_from_struct(
# 		self,
# 		multistate_dict: Dict[str, str]
# 		):
# 		"""
# 		Extract and save the required chain from the pdb/cif file.
# 		Firxt, get a chain mapping between the asym_id and auth_asym_id.
# 			The input file contains asym_id, which works fine for CIF format.
# 			For PDB format, we need the auth_asym_id corresponding to the asym_id.
# 		"""
# 		for sys_name in multistate_dict:
# 			sys_dir_path = get_sys_data_dir_path(
# 				base_dir = self.base_dir,
...
# 				sys_name = sys_name
# 			)
# 			for i, state in enumerate( ["apo", "holo"], start = 1 ):
# 				pdb_id = multistate_dict[sys_name][state]["pdb_id"]
# 				chain_id = multistate_dict[sys_name][state]["chain_id"]
# 				pdb_file = os.path.join(
# 						self.dir_paths["pdb_dir"], f"{pdb_id}"
# 					)
# 				mmcif_obj = MmcifDictParser( cif_file = pdb_file + ".cif" )
# 				chain_mapping = mmcif_obj.get_chain_mapping()
# 				# for ext in ["cif", "pdb"]:
# 				for ext in ["pdb"]:
...
# 					)
# 					if ext == "pdb":
# 						# Get the auth_asym_id for PDB format.
# 						output_chain = chain_mapping["asym"][chain_id]
# 					else:
# 						output_chain = chain_id
# 					extract_chains(
# 						struct_file = pdb_file + f".{ext}",
# 						chain_id = output_chain,
# 						ext = ext,
# 						output_file = state_file
# 					)
# 					# Add state file path to multistate_dict.
# 					multistate_dict[sys_name][state]["state_file"] = state_file
# 		# exit()
# 		return multistate_dict
...
# 	):
# 		"""
# 		Remove XLs which are satisfied by the other state.
# 			e.g. Remove state1 XLs satisfied by state2 PDB file.
# 		"""
# 		coords = {}
# 		struct = Parser( pdb_file = pdb_file ).structure
# 		for chain in struct[0]:
...
# 			# Both structures should have the residue
...
# 			xl_df = xls_s2,
# 			pdb_num_seq_id_map = pdb_num_seq_id_map
# 		)
...
# 	def select_rep_seq(
# 		self,
# 		sys_metadata: Dict[str, Dict]
# 	) -> Tuple[str, int, int]:
# 		"""
# 		Given the sequence for apo and holo states for the system,
# 			select the representative sequence to be modeled and
# 			the corresponding residue positions.
# 		The smaller of the apo and holo seq is considered the
# 			representative.
# 		"""
# 		state1_seq = sys_metadata["apo"]["seq"]
# 		state2_seq = sys_metadata["holo"]["seq"]

# 		if len( state1_seq ) <= len( state2_seq ):
# 			rep_seq = state1_seq
# 			state = "apo"
# 		elif len( state2_seq ) < len( state1_seq ):
# 			rep_seq = state2_seq
# 			state = "holo"

# 		start_res = sys_metadata[state]["start_seq_id"]
# 		end_res = sys_metadata[state]["end_seq_id"]
# 		seq_id = sys_metadata[state]["seq_id"]

# 		if seq_id[0] != start_res or seq_id[-1] != end_res:
# 			raise ValueError(
# 				f"Start {start_res} and end {end_res} residues do not " +
# 				"match the seq_id residue positions..."
# 			)

# 		return rep_seq, start_res, end_res
...
# 		sys_configs = {}
# 		for sys_name in multistate_dict:
# 			rep_seq, start_res, end_res = self.select_rep_seq(
# 				sys_metadata = multistate_dict[sys_name]
# 			)
...
# 			r1_idx = np.where( residue_positions == r1 )
# 			r2_idx = np.where( residue_positions == r2 )

# 			if len( r1_idx ) == 0 or len( r2_idx ) == 0:
# 				raise ValueError( f"Residue not found: {r1}, index = {r1_idx}..." )
# 			r1_idx = r1_idx[0]
# 			r2_idx = r2_idx[0]

# 			coord1 = coords[r1_idx]
# 			coord2 = coords[r2_idx]
...
# 				sys_name = sys_name
# 			)
# 			# print( multistate_dict[sys_name]["holo"]["mapping"]["A"].keys() )
# 			for k, v in multistate_dict[sys_name]["apo"]["mapping"]["A"].items():
# 				if v == 31:
# 					print( k, "  ", v )
# 			exit()
# 			state1_pdb = os.path.join( sys_dir_path, f"{sys_name}_S1.pdb" )
...
# 			# Only a single seq here
# 			start_res = sys_configs[sys_name]["entity"][0]["start"]
# 			end_res = sys_configs[sys_name]["entity"][0]["end"]
...
# 					residue_positions = np.arange( start_res, end_res+1, 1 )

# 					xl_sat = self.compute_xl_satisfaction(
# 						coords = coords,
# 						xl_df = xl_df,
...
# 			print( sys_name, "-"*20 )
# 			for k in xl_sat_dict[sys_name]:
# 				print( f"\t{xl_sat_dict[sys_name][k]}" )
# 		return xl_sat_dict
...
if __name__ == "__main__":
	...
	# _ = ApoHoloStates(
	# 	apo_cif_file = "/data2/kartik/IMP_Rewired/im_bench/imp_dl/benchmark/multistate_metadata/struct/1vr6.cif",
	# 	holo_cif_file = "/data2/kartik/IMP_Rewired/im_bench/imp_dl/benchmark/multistate_metadata/struct/1rzm.cif",
	# 	apo_asym_id = "A",
	# 	holo_asym_id = "A",
	# 	apo_out_file = "./apo.pdb",
	# 	holo_out_file = "./holo.pdb"
	# ).forward()
